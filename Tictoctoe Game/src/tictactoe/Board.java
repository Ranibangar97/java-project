...
public class Board {
  ...
  public int move(char symbol,int x,int y) { //x and y are the coordinates where the symbol added on 2d board
		if(x<0 || x>=boardSize || y<0 || y>=boardSize|| board[x][y]!= EMPTY) {
			return INVALIDMOVE;
		}
		board[x][y]=symbol;
		count++;      //count filled cells
		
		//check row cells contains same symbol
		if(board[x][0]==board[x][1] && board[x][0]==board[x][2]) {
			return symbol == p1Symbol ? PLAYER1WINS :PLAYER2WINS;
		}
		//check col cells contains same symbol
		if(board[0][y]==board[1][y] && board[0][y]==board[2][y]) {
			return symbol == p1Symbol ? PLAYER1WINS :PLAYER2WINS;
		}
		//check diagonal cells contains same symbol
		//first diagonal
